...
public:
    int uniquePathsWithObstacles(vector<vector<int>>& obstacleGrid) {
        ...
        std::for_each(++std::cbegin(obstacleGrid), std::cend(obstacleGrid), [&dp](auto const row){
            dp.front() *= 1 - row.front();
            for (std::size_t column{1}; column != std::size(row); ++column) dp.at(column) = (dp.at(column) + dp.at(column - 1)) * (1 - row.at(column));});
        return dp.back();
    }
};
