...
public:
    vector<string> letterCombinations(string digits) {
        ...
        return std::empty(digits) ? type{} : std::accumulate(std::cbegin(digits), std::cend(digits), std::vector{""s} ,[&phone = std::as_const(phone)](auto const result, auto const digit){
            type out;
